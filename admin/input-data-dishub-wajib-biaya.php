                              ...
                              if ($handle) {
                                   ...
                                   while (($data = fgetcsv($handle, 1000, ";")) !== false) {
                                        ...
                                        if (count($data) === 3) {
                                             ...
                                             $jumlah = (int)$conn->real_escape_string($data[1]);
                                             $biaya = (float)$conn->real_escape_string($data[2]);
                                             $idOpd = $conn->real_escape_string($opd);

                                             // Query untuk menyimpan data ke dalam tabel

                                             $stmt = $conn->prepare("INSERT INTO dishub_wajib_biaya (id_bulan, bulan, jumlah, biaya, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?)");
                                             $stmt->bind_param("ssdisi", $uniqueId, $bulan, $jumlah, $biaya, $idOpd, $tahun);
                                             $result = $stmt->execute();
                                        }
                                   }
