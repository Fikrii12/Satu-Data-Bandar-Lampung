							...
							if ($handle) {
								...
								while (($data = fgetcsv($handle, 1000, ";")) !== false) {
									...
									if (count($data) === 4) {
										...
										$luas_tanaman = $conn->real_escape_string($data[1]);
										$luas_rusak = $conn->real_escape_string($data[2]); 
										$luas_penanaman = $conn->real_escape_string($data[3]); 

										// Query untuk menyimpan data ke dalam tabel
										
										$stmt = $conn->prepare("INSERT INTO dispentani (id_tanaman, nama_tanaman, luas_tanaman, luas_rusak, luas_penanaman, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
										$stmt->bind_param("sssssss", $uniqueId, $nama, $luas_tanaman, $luas_rusak, $luas_penanaman, $opd, $tahun);
										$result = $stmt->execute();
									}
								}
