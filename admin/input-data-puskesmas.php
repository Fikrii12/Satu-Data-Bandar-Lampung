						...
						<div class="clearfix">
							...
							if ($handle) {
								...
								while (($data = fgetcsv($handle, 1000, ";")) !== false) {
									...
									if (count($data) === 3) {
										$id_sarana = uniqid();
										$sarana = $conn->real_escape_string($data[0]);
										$rawatInap = $conn->real_escape_string($data[1]);
										$nonRawatInap = $conn->real_escape_string($data[2]);

										// Query untuk menyimpan data ke dalam tabel
										$stmt = $conn->prepare("INSERT INTO puskesmas (id_sarana, sarana, rawat_inap, non_rawat_inap, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?)");
										$stmt->bind_param("ssssss", $id_sarana, $sarana, $rawatInap, $nonRawatInap, $opd, $tahun);
										$result = $stmt->execute();
									}
								}
