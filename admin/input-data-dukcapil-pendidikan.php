							...
							<div class="pull-left">
                              ...
                              <?php
                                   ...
                                   if ($handle) {
                                        ...
                                        $opd = "Dinas Dukcapil"; // Update this value to match the correct OPD value in your case.
                                        while (($data = fgetcsv($handle, 1000, ";")) !== false) {
                                             ...
                                             // Modify the following section to match the columns of dukcapil_pendidikan table.
                                             if (count($data) === 12) {
                                                  $id_kecamatan = uniqid();
                                                  $kecamatan = $conn->real_escape_string($data[0]);
                                                  $tidak_sekolah = $conn->real_escape_string($data[1]);
                                                  $belum_sd = $conn->real_escape_string($data[2]);
                                                  $tamat_sd = $conn->real_escape_string($data[3]);
                                                  $sltp = $conn->real_escape_string($data[4]);
                                                  $slta = $conn->real_escape_string($data[5]);
                                                  $d1_2 = $conn->real_escape_string($data[6]);
                                                  $d3 = $conn->real_escape_string($data[7]);
                                                  $d4_s1 = $conn->real_escape_string($data[8]);
                                                  $s2 = $conn->real_escape_string($data[9]);
                                                  $s3 = $conn->real_escape_string($data[10]);
                                                  $jumlah = $conn->real_escape_string($data[11]);
                                 

                                                  // Query untuk menyimpan data ke dalam tabel
                                                  $stmt = $conn->prepare("INSERT INTO dukcapil_pendidikan (id_kecamatan, kecamatan, tidak_sekolah, belum_sd, tamat_sd, sltp, slta, d1_2, d3, d4_s1, s2, s3, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                                                  $stmt->bind_param("ssiiiiiiiiiiisi", $id_kecamatan, $kecamatan, $tidak_sekolah, $belum_sd, $tamat_sd, $sltp, $slta, $d1_2, $d3, $d4_s1, $s2, $s3, $jumlah, $opd, $tahun);
                                                  $result = $stmt->execute();
