							...
							if ($handle) {
								...
								while (($data = fgetcsv($handle, 1000, ";")) !== false) {
									...
									if (count($data) === 12) {
										...
										$uniqueId = uniqid();
										$golongan = $conn->real_escape_string($data[0]); 
										$umur1 = $conn->real_escape_string($data[1]); 
										$umur2 = $conn->real_escape_string($data[2]); 
										$umur3 = $conn->real_escape_string($data[3]); 
										$umur4 = $conn->real_escape_string($data[4]); 
										$umur5 = $conn->real_escape_string($data[5]); 
										$umur6 = $conn->real_escape_string($data[6]); 
										$umur7 = $conn->real_escape_string($data[7]); 
										$umur8 = $conn->real_escape_string($data[8]); 
										$umur9 = $conn->real_escape_string($data[9]); 
										$umur10 = $conn->real_escape_string($data[10]); 
										$jumlah = $umur1+$umur2+$umur3+$umur4+$umur5+$umur6+$umur7+$umur8+$umur9+$umur10; 
										

										// Query untuk menyimpan data ke dalam tabel
										
                                                  $stmt = $conn->prepare("INSERT INTO bkd_golongan_umur (id_golongan, golongan, umur1, umur2, umur3, umur4, umur5, umur6, umur7, umur8, umur9, umur10, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                                                  $stmt->bind_param("sssssssssssssss", $uniqueId, $golongan, $umur1, $umur2, $umur3, $umur4, $umur5, $umur6, $umur7, $umur8, $umur9, $umur10, $jumlah, $opd, $tahun);
                                                  $result = $stmt->execute();
									}
								}
