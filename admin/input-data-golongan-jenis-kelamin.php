							...
							if ($handle) {
								...
								while (($data = fgetcsv($handle, 1000, ";")) !== false) {
									...
									if (count($data) === 4) {
										...
										$jumlahPria = $conn->real_escape_string($data[1]); 
										$jumlahWanita = $conn->real_escape_string($data[2]); 
										$jumlahTotal = $conn->real_escape_string($data[3]); 

										// Query untuk menyimpan data ke dalam tabel
										
										$stmt = $conn->prepare("INSERT INTO bkd_golongan_jenis_kelamin (id_golongan_jenis_kelamin, golongan, pria, wanita, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
										$stmt->bind_param("sssssss", $uniqueId, $golongan, $jumlahPria, $jumlahWanita, $jumlahTotal, $opd, $tahun);
										$result = $stmt->execute();
									}
								}
