							...
							if ($handle) {
								...
								while (($data = fgetcsv($handle, 1000, ";")) !== false) {
									...
									if (count($data) === 4) {
										...
										$uniqueId = uniqid();
										$komoditas = $conn->real_escape_string($data[0]); // Kolom 1 berisi nama makanan
										$total = $conn->real_escape_string($data[1]); // Kolom 2 berisi jumlah kalori
										$kebutuhan = $conn->real_escape_string($data[2]); // Kolom 2 berisi jumlah kalori
										$stok_akhir= $conn->real_escape_string($data[3]); // Kolom 2 berisi jumlah kalori

										// Query untuk menyimpan data ke dalam tabel
										
										$stmt = $conn->prepare("INSERT INTO dispan (id_komoditas, komoditas, total, kebutuhan, stok_akhir, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
										$stmt->bind_param("sssssss", $uniqueId, $komoditas, $total, $kebutuhan, $stok_akhir, $opd, $tahun);
										$result = $stmt->execute();
									}
								}
