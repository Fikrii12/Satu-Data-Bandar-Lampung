						...
						<?php
                                   ...
                                   if ($handle) {
                                        ...
                                        while (($data = fgetcsv($handle, 1000, ";")) !== false) {
                                             ...
                                             // Modify the following section to match the columns of dukcapil_agama table.
                                             if (count($data) === 9) {
                                                  $id_kecamatan = uniqid();
                                                  $kecamatan = $conn->real_escape_string($data[0]);
                                                  $islam = $conn->real_escape_string($data[1]);
                                                  $kristen = $conn->real_escape_string($data[2]);
                                                  $katolik = $conn->real_escape_string($data[3]);
                                                  $hindu = $conn->real_escape_string($data[4]);
                                                  $budha = $conn->real_escape_string($data[5]);
                                                  $konghucu = $conn->real_escape_string($data[6]);
                                                  $kepercayaan = $conn->real_escape_string($data[7]);
                                                  $jumlah = $conn->real_escape_string($data[8]);

                                                  // Query untuk menyimpan data ke dalam tabel

                                                  $stmt = $conn->prepare("INSERT INTO dukcapil_agama (id_kecamatan, kecamatan, islam, kristen, katolik, hindu, budha, konghucu, kepercayaan, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                                                  $stmt->bind_param("ssiiiiiiiiii", $id_kecamatan, $kecamatan, $islam, $kristen, $katolik, $hindu, $budha, $konghucu, $kepercayaan, $jumlah, $opd, $tahun);
                                                  $result = $stmt->execute();
                                             }
                                        }
