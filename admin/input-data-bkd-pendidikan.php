						...
						<?php
							...
							if ($handle) {
								...
								while (($data = fgetcsv($handle, 1000, ";")) !== false) {
									...
									// Pastikan array data memiliki 2 elemen (nama makanan dan kalori)
									if (count($data) === 4) {
										// Generate unique ID for the food name
										$uniqueId = uniqid();
										$tingkatPendidikan = $conn->real_escape_string($data[0]); // Kolom 1 berisi nama makanan
										$jumlahPria = $conn->real_escape_string($data[1]); // Kolom 2 berisi jumlah kalori
										$jumlahWanita = $conn->real_escape_string($data[2]); // Kolom 2 berisi jumlah kalori
										$jumlahTotal = $conn->real_escape_string($data[3]); // Kolom 2 berisi jumlah kalori

										// Query untuk menyimpan data ke dalam tabel
										
										$stmt = $conn->prepare("INSERT INTO bkd_pendidikan (id_pendidikan, pendidikan, pria, wanita, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
										$stmt->bind_param("sssssss", $uniqueId, $tingkatPendidikan, $jumlahPria, $jumlahWanita, $jumlahTotal, $opd, $tahun);
										$result = $stmt->execute();
									}
								}
