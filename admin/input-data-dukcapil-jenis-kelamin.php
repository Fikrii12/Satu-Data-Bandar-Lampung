                              ...
                              <?php
                                   ...
                                   if ($handle) {
                                        ...
                                        while (($data = fgetcsv($handle, 1000, ";")) !== false) {
                                             ...
                                             // Modify the following section to match the columns of dukcapil_jenis_kelamin table.
                                             if (count($data) === 4) {
                                                  $id_kecamatan = uniqid();
                                                  $kecamatan = $conn->real_escape_string($data[0]);
                                                  $pria = $conn->real_escape_string($data[1]);
                                                  $wanita = $conn->real_escape_string($data[2]);
                                                  $jumlah = $conn->real_escape_string($data[3]);

                                                  // Query untuk menyimpan data ke dalam tabel
                                                  $stmt = $conn->prepare("INSERT INTO dukcapil_jenis_kelamin (id_kecamatan, kecamatan, pria, wanita, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
                                                  $stmt->bind_param("ssiiisi", $id_kecamatan, $kecamatan, $pria, $wanita, $jumlah, $opd, $tahun);
                                                  $result = $stmt->execute();
