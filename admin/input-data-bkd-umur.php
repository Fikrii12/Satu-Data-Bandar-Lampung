							...
							if ($handle) {
								...
								while (($data = fgetcsv($handle, 1000, ";")) !== false) {
									...
									if (count($data) === 4) {
										...
										$uniqueId = uniqid();
										$umur = $conn->real_escape_string($data[0]); // Kolom 1 berisi nama makanan
										$pria = $conn->real_escape_string($data[1]); // Kolom 2 berisi jumlah kalori
										$wanita = $conn->real_escape_string($data[2]); // Kolom 2 berisi jumlah kalori
										$juamlah = $conn->real_escape_string($data[3]); // Kolom 2 berisi jumlah kalori

										// Query untuk menyimpan data ke dalam tabel
										
										$stmt = $conn->prepare("INSERT INTO bkd_umur (id_umur, umur, pria, wanita, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
										$stmt->bind_param("sssssss", $uniqueId, $umur, $pria, $wanita, $juamlah, $opd, $tahun);
										$result = $stmt->execute();
									}
								}
