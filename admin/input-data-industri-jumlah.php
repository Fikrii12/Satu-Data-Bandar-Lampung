								...
								<nav aria-label="breadcrumb" role="navigation">
									<ol class="breadcrumb">
										<li class="breadcrumb-item">
											<a href="table-perindustrian.php">Data Dinas Perindustrian</a>
										</li>
						...
						<?php
                                   ...
                                   if ($handle) {
                                        ...
                                        while (($data = fgetcsv($handle, 1000, ";")) !== false) {
                                             ...
                                             if (count($data) === 4) {
                                                  $uniqueId = uniqid();
                                                  $industri = $conn->real_escape_string($data[0]);
                                                  $jumlah = $conn->real_escape_string($data[1]);
                                                  $ikahh = $conn->real_escape_string($data[2]);
                                                  $ilmea = $conn->real_escape_string($data[3]);

                                                  // Query untuk menyimpan data ke dalam tabel
                                                  $stmt = $conn->prepare("INSERT INTO industri_jumlah (id_industri, industri, jumlah, ikahh, ilmea, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
                                                  $stmt->bind_param("sssssss", $uniqueId, $industri, $jumlah, $ikahh, $ilmea, $opd, $tahun);
                                                  $result = $stmt->execute();
                                             }
                                        }
