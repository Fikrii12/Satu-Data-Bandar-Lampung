						...
						if ($handle) {
							...
							while (($data = fgetcsv($handle, 1000, ";")) !== false) {
								...
								if (count($data) === 2) {
									...
									$uniqueId = uniqid();
									$jenis = $conn->real_escape_string($data[0]);
									$jumlah = (int)$conn->real_escape_string($data[1]);

									// Query untuk menyimpan data ke dalam tabel

									$stmt = $conn->prepare("INSERT INTO dishub_wajib_noumum (id_kendaraan, jenis, jumlah, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?)");
									$stmt->bind_param("ssiss", $uniqueId, $jenis, $jumlah, $opd, $tahun);
									$result = $stmt->execute();
								}
							}
