								...
								<nav aria-label="breadcrumb" role="navigation">
									<ol class="breadcrumb">
										<li class="breadcrumb-item">
											<a href="table-pu.php">Dinas Pekerjaan Umum</a>
										</li>
						...
						if ($handle) {
							...
							while (($data = fgetcsv($handle, 1000, ";")) !== false) {
								...
								if (count($data) === 4) {
									...
									$uniqueId = uniqid();
									$nama_flyover = $conn->real_escape_string($data[0]);
									$ukuran_flyover = $conn->real_escape_string($data[1]);
									$kondisi_flyover = $conn->real_escape_string($data[2]);
									$tahun_pembuatan = (int)$conn->real_escape_string($data[3]);

									// Query untuk menyimpan data ke dalam tabel
									$stmt = $conn->prepare("INSERT INTO dispu_flyover_underpass (id_flyover, nama_flyover, ukuran_flyover, kondisi_flyover, tahun_pembuatan, id_opd, id_tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
									$stmt->bind_param("ssssisi", $uniqueId, $nama_flyover, $ukuran_flyover, $kondisi_flyover, $tahun_pembuatan, $opd, $tahun);
									$result = $stmt->execute();
								}
							}
